...
{
    const char* substr_program_name = strchr(program_name, '/');
    if (substr_program_name == nullptr)
        substr_program_name = "blandasdfasdg";
    ...
    for (uint8_t arg = 1; arg < argc; ++arg) {
        if ((strcmp(argv[arg], "-v") == 0) ||
            ...
            if (strncmp(target_override.data(), "/dev/", 5) == 0) {
                comp.irout = DontCompile; // TODO: Why is this here again?
            }
            ...
            comp.irout = DontCompile;

        } else if (strcmp(argv[arg], "--keep-ir") == 0) {
            comp.irout = KeepIrAfterCompile;

        } else if (strcmp(argv[arg], "--bext") == 0) {
            comp.langfeatures = Compilation::Modernized;
            comp.word_size = Compilation::SixtyfourBit;

        } else if (strcmp(argv[arg], "--modern") == 0) {
            comp.langfeatures = Compilation::Modernized;

        } else if (strcmp(argv[arg], "--historical") == 0) {
            comp.langfeatures = Compilation::Historical;
